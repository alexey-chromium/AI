...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (G, x, x_norm, mean, var, gamma_reshaped, beta_reshaped, gamma_shape, beta_shape, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    G, x, x_norm, mean, var, gamma, beta, gamma_shape, beta_shape, eps = cache

    N, C, H, W = x.shape
    M = (C // G) * H * W

    dbeta = np.sum(dout, axis=(0, 2, 3), keepdims=True).reshape(beta_shape)
    dgamma = np.sum(dout * x_norm, axis=(0, 2, 3), keepdims=True).reshape(gamma_shape)
    ...
    dx_group = (1.0 / M) * std_inv * (
        M * dx_norm_group
        - np.sum(dx_norm_group, axis=(2, 3, 4), keepdims=True)
        - x_norm_group * np.sum(dx_norm_group * x_norm_group, axis=(2, 3, 4), keepdims=True)
    )
